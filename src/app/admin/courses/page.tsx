...
export default async function AdminCoursesPage() {
  ...
  const courses: AdminCourseRow[] = await prisma.course.findMany({
  ...
  return (
    <section className="mx-auto w-full max-w-6xl space-y-6">
      ...
      <div className="space-y-4">
        {courses.map((course: AdminCourseRow) => (
          <form
            key={course.id}
            action={updateCourse}
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-white/70">
                  {(course.category?.name ?? "بدون تصنيف")} • المدرب:{" "}
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  className="rounded-xl bg-[#9e28b5] px-3 py-2 text-xs font-semibold"
                  type="submit"
                >
                  تحديث الحالة
                </button>
